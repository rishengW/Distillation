...
# Early stopping: halt training if validation perplexity fails to improve for
# `EARLY_STOP_PATIENCE` consecutive epochs by at least `EARLY_STOP_MIN_DELTA`.
# When triggered, the best-PPL checkpoint (not the last) is what gets saved.
# Set EARLY_STOP_PATIENCE = 0 to disable.
EARLY_STOP_PATIENCE   = 2
EARLY_STOP_MIN_DELTA  = 0.01    # absolute PPL improvement required to "count"
...
best_ppl       = float("inf")
epochs_no_improve = 0
best_state     = None       # holds the best student state_dict (CPU tensors)
stopped_early  = False
...
for epoch in range(1, EPOCHS + 1):
    ...
    # ── Early stopping ────────────────────────────────────────────────────────
    if val_ppl < best_ppl - EARLY_STOP_MIN_DELTA:
        best_ppl = val_ppl
        epochs_no_improve = 0
        # Snapshot the best weights to CPU so a later overfit epoch can't lose them.
        best_state = {k: v.detach().cpu().clone() for k, v in student.state_dict().items()}
        print(f"  ↳ new best val_ppl={best_ppl:.2f}  (checkpoint snapshotted)\n")
    else:
        epochs_no_improve += 1
        print(f"  ↳ no improvement for {epochs_no_improve}/{EARLY_STOP_PATIENCE} epoch(s)  "
              f"(best={best_ppl:.2f})\n")
        if EARLY_STOP_PATIENCE > 0 and epochs_no_improve >= EARLY_STOP_PATIENCE:
            print(f"Early stopping at epoch {epoch}: val_ppl has not improved "
                  f"for {EARLY_STOP_PATIENCE} consecutive epoch(s).\n")
            stopped_early = True
            break
...
if best_state is not None:
    ...
    print(f"Restored best student weights (val_ppl={best_ppl:.2f}) for save & final eval.")
del best_state
...
print(f"PPL gap      : {final_ppl - teacher_ppl:+.2f}")
print(f"Param reduction : {(1 - student_params / teacher_params) * 100:.1f}%")
>>>>>>> 313dc8ea844e8a4309ca3edce0354e308b8e3c57
