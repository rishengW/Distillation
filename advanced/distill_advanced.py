...
# ── Qwen-specific MiniLMv2 wrapper ────────────────────────────────────────────
# minilm_relation_kl in losses.py is model-agnostic — it just takes Q/K/V
# tensors. Here we extract Qwen2's last-layer Q from the cached hidden_states
# (no extra forward pass) and feed them in.
def _qwen_last_q(model, hidden_states):
    """
    Extract Q at the last transformer layer of a Qwen2 model.
...
def minilm_q_relation_loss(
    ...
    """
    s_q = _qwen_last_q(student_model, student_hidden_states)
    with torch.no_grad():
        t_q = _qwen_last_q(teacher_model, teacher_hidden_states)
